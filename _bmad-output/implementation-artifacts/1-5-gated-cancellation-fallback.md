...
> **OQ1 RESOLVED — follow-up done (2026-07-15).** Real cancelled-service fixtures were captured from live HSP data (`tests/fixtures/recorded_details_cancelled_202607107679744.json` — the 2026-07-10 18:30 Waterloo→Portsmouth Harbour, cancelled at Waterloo, `late_canc_reason` "911"; plus its next-catchable `recorded_details_catchable_202607107679102.json`). `tests/test_cancellation_recorded.py` adds `@recorded` scenarios proving the real JSON maps to `Service.cancelled=True` and that the fallback derives the correct next-catchable delay (18 min → band 15-29). Per the story's own plan ("when OQ1 resolves, a follow-up enables the flag and adds a `@recorded` scenario"), the AD-6 gate is now **on by default** in `RunConfig`; the CLI `--no-cancellations` flag opts out. The pure `optimise(...)` function keeps its own default OFF, so synthetic AC1/AC2 tests are unchanged.
